...
public interface PeerManager {
   ...
   * Peer must occur in this stream strictly after occurring in the {@link #connectedPeerStream()}
   */
  Publisher<Peer> disconnectedPeerStream();

  /**
   * Steam of new active peers which are connected and handshake was done.
   * A peer appearing in this stream is available for 'high-level' APIs calls
   */
  ...
  /**
   * Returns WireApiSync instance which is the aggregation of all connected peer APIs
   * When currently no active peers the API enqueues invocations and execute them upon any
   * active peer connected
   */
  WireApiSync getWireApiSync();

  /**
   * Returns WireApiSub instance which is the aggregation of all connected peer APIs.
   * When currently no active peers the instance just ignores outbound notifications.
   */
