...
public class DiscoveryNoNetworkTest {
  ...
  public void test() throws Exception {
    // 1) start 2 nodes
    NodeRecordV5 nodeRecord1 = NodeRecordV5.Builder.empty()
        .withIpV4Address(BytesValue.wrap(InetAddress.getByName("127.0.0.1").getAddress()))
        .withSeqNumber(1L)
        .withUdpPort(30303)
        .withSecp256k1(BytesValue.fromHexString("0bfb48004b1698f05872cf18b1f278998ad8f7d4c135aa41f83744e7b850ab6b98"))
        .build();
    NodeRecordV5 nodeRecord2 = NodeRecordV5.Builder.empty()
        .withIpV4Address(BytesValue.wrap(InetAddress.getByName("192.168.0.1").getAddress()))
        .withSeqNumber(1L)
        .withUdpPort(30303)
        .withSecp256k1(BytesValue.fromHexString("7ef3502240a42891771de732f5ee6bee3eb881939edf3e6008c0d07b502756e426"))
        .build();
    ...
    NodeTableStorage nodeTableStorage1 = nodeTableStorageFactory.create(database1, serializerFactory, () -> nodeRecord1, () -> new ArrayList<NodeRecord>() {{add(nodeRecord2);}});
    NodeTableStorage nodeTableStorage2 = nodeTableStorageFactory.create(database2, serializerFactory, () -> nodeRecord2, () -> new ArrayList<NodeRecord>() {{add(nodeRecord1);}});
    SimpleProcessor<UnknownPacket> from1to2 = new SimpleProcessor<>(Schedulers.createDefault().newSingleThreadDaemon("from1to2-thread"), "from1to2");
    SimpleProcessor<UnknownPacket> from2to1 = new SimpleProcessor<>(Schedulers.createDefault().newSingleThreadDaemon("from2to1-thread"), "from2to1");
    DiscoveryManager discoveryManager1 = new DiscoveryManager(
        nodeTableStorage1.get(),
        from2to1,
        NODE_ID_FUNCTION.apply(nodeRecord1)
    );
    DiscoveryManager discoveryManager2 = new DiscoveryManager(
        nodeTableStorage2.get(),
        from1to2,
        NODE_ID_FUNCTION.apply(nodeRecord2)
    );

    discoveryManager1.start();
    discoveryManager2.start();
    // 2) Link outgoing of each one with incoming of another
    Flux.from(discoveryManager1.getOutgoingMessages()).subscribe(t -> from1to2.onNext(new UnknownPacket(t.getPacket().getBytes())));
    Flux.from(discoveryManager2.getOutgoingMessages()).subscribe(t -> from2to1.onNext(new UnknownPacket(t.getPacket().getBytes())));
    ...
    Flux.from(from1to2).subscribe(networkPacket -> {
      // 1 -> 2 random
      if (randomSent1to2.getCount() != 0) {
        ...
        AuthHeaderMessagePacket authHeaderMessagePacket = networkPacket.getAuthHeaderMessagePacket();
    ...
    Flux.from(from2to1).subscribe(networkPacket -> {
      // 2 -> 1 whoareyou
