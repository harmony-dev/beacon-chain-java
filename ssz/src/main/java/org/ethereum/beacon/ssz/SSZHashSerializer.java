...
public class SSZHashSerializer extends SSZSerializer {
  ...
  public SSZHashSerializer(SSZSchemeBuilder schemeBuilder, SSZCodecResolver codecResolver, SSZModelFactory sszModelFactory) {
  ...
  public byte[] encode(@Nullable Object input, Class clazz) {
    ...
    // For the final output only (ie. not intermediate outputs), if the output is less than 32 bytes, right-zero-pad it to 32 bytes.
    byte[] res;
