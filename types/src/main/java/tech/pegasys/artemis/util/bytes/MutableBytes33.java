...
/**
 * A mutable {@link Bytes33}, that is a mutable {@link BytesValue} of exactly 33 bytes.
 */
public interface MutableBytes33 extends MutableBytesValue, Bytes33 {

  /**
   * Wraps a 33 bytes array as a mutable 33 bytes value.
   *
   * <p>
   * This method behave exactly as {@link Bytes33#wrap(byte[])} except that the result is a mutable.
   *
   * @param value The value to wrap.
   * @return A {@link MutableBytes33} wrapping {@code value}.
   * @throws IllegalArgumentException if {@code value.length != 33}.
   */
  static MutableBytes33 wrap(byte[] value) {
    return new MutableArrayWrappingBytes33(value);
  }

  /**
   * Creates a new mutable 33 bytes value.
   *
   * @return A newly allocated {@link MutableBytesValue}.
   */
  static MutableBytes33 create() {
    return new MutableArrayWrappingBytes33(new byte[SIZE]);
  }

  /**
   * Wraps an existing {@link MutableBytesValue} of size 33 as a mutable 33 bytes value.
   *
   * <p>
  ...
  static MutableBytes33 wrap(MutableBytesValue value) {
    ...
    return new MutableBytes33() {
      ...
      @Override
      public Bytes33 copy() {
        return Bytes33.wrap(value.extractArray());
      }

      @Override
      public MutableBytes33 mutableCopy() {
        return MutableBytes33.wrap(value.extractArray());
      }
