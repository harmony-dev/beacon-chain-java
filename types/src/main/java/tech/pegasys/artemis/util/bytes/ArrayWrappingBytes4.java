...
/**
 * An implementation of {@link Bytes4} backed by a byte array ({@code byte[]}).
 */
class ArrayWrappingBytes4 extends ArrayWrappingBytesValue implements Bytes4 {

  ArrayWrappingBytes4(byte[] bytes) {
    this(checkLength(bytes), 0);
  }

  ArrayWrappingBytes4(byte[] bytes, int offset) {
