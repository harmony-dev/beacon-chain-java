...
/**
 * A mutable {@link Bytes8}, that is a mutable {@link BytesValue} of exactly 8 bytes.
 */
public interface MutableBytes8 extends MutableBytesValue, Bytes8 {

  /**
   * Wraps a 8 bytes array as a mutable 8 bytes value.
   *
   * <p>
   * This method behave exactly as {@link Bytes8#wrap(byte[])} except that the result is a mutable.
   *
   * @param value The value to wrap.
   * @return A {@link MutableBytes8} wrapping {@code value}.
   * @throws IllegalArgumentException if {@code value.length != 8}.
   */
  static MutableBytes8 wrap(byte[] value) {
    return new MutableArrayWrappingBytes8(value);
  }

  /**
   * Creates a new mutable 8 bytes value.
   *
   * @return A newly allocated {@link MutableBytesValue}.
   */
  static MutableBytes8 create() {
    return new MutableArrayWrappingBytes8(new byte[SIZE]);
  }

  /**
   * Wraps an existing {@link MutableBytesValue} of size 8 as a mutable 8 bytes value.
   *
   * <p>
  ...
  static MutableBytes8 wrap(MutableBytesValue value) {
    ...
    return new MutableBytes8() {
      ...
      @Override
      public Bytes8 copy() {
        return Bytes8.wrap(value.extractArray());
      }

      @Override
      public MutableBytes8 mutableCopy() {
        return MutableBytes8.wrap(value.extractArray());
      }
