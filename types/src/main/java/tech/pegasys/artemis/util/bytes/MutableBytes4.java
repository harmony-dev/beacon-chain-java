...
/**
 * A mutable {@link Bytes4}, that is a mutable {@link BytesValue} of exactly 4 bytes.
 */
public interface MutableBytes4 extends MutableBytesValue, Bytes4 {

  /**
   * Wraps a 4 bytes array as a mutable 4 bytes value.
   *
   * <p>
   * This method behave exactly as {@link Bytes4#wrap(byte[])} except that the result is a mutable.
   *
   * @param value The value to wrap.
   * @return A {@link MutableBytes4} wrapping {@code value}.
   * @throws IllegalArgumentException if {@code value.length != 4}.
   */
  static MutableBytes4 wrap(byte[] value) {
    return new MutableArrayWrappingBytes4(value);
  }

  /**
   * Creates a new mutable 4 bytes value.
   *
   * @return A newly allocated {@link MutableBytesValue}.
   */
  static MutableBytes4 create() {
    return new MutableArrayWrappingBytes4(new byte[SIZE]);
  }

  /**
   * Wraps an existing {@link MutableBytesValue} of size 4 as a mutable 4 bytes value.
   *
   * <p>
  ...
  static MutableBytes4 wrap(MutableBytesValue value) {
    ...
    return new MutableBytes4() {
      ...
      public MutableBytes4 mutableCopy() {
        return MutableBytes4.wrap(value.extractArray());
      }
