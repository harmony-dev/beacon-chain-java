...
public abstract class BytesValues {
   ...
   */
  public static int countZeros(BytesValue value) {
    int count = 0;
   ...
   */
  public static int extractInt(BytesValue value) {
    int size = value.size();
    ...
    for (int i = 0; i < size; i++) {
      res |= ((int) value.get(size - i - 1) & 0xFF) << shift;
      shift += 8;
    }
    return res;
  }
   ...
   */
  public static long extractLong(BytesValue value) {
    int size = value.size();
  ...
  public static BytesValue concatenate(BytesValue... values) {
    int size = 0;
    for (BytesValue value : values) {
      size += value.size();
    }

    MutableBytesValue result = MutableBytesValue.create(size);
    int offset = 0;
    for (BytesValue value : values) {
   ...
   */
  public static String asString(BytesValue bytes) {
  ...
  private static int leadingZeros(BytesValue bytes) {
