 ...
 * @see BeaconBlock
 * @see <a
 *     href="https://github.com/ethereum/eth2.0-specs/blob/v0.9.2/specs/core/0_beacon-chain.md#beacon-state">BeaconState
 *     in the spec</a>
 */
public interface BeaconState extends ObservableComposite {
  ...
  static BeaconState getEmpty(SpecConstants specConst) {
    ...
    ret.getStateRoots().addAll(
        Collections.nCopies(specConst.getSlotsPerHistoricalRoot().intValue(), Hash32.ZERO));
    ret.getSlashings().addAll(
  ...
  @SSZ(order = 0) Time getGenesisTime();

  /** Slot number that this state was calculated in. */
  @SSZ(order = 1) SlotNumber getSlot();
  ...
  @SSZ(order = 3) BeaconBlockHeader getLatestBlockHeader();
  ...
  @SSZ(order = 12, vectorLengthVar = "spec.EPOCHS_PER_HISTORICAL_VECTOR")
  ReadVector<EpochNumber, Hash32> getRandaoMixes();

  /* ******** Slashings ********* */

  /** Per-epoch sums of slashed effective balances. */
  @SSZ(order = 13, vectorLengthVar = "spec.EPOCHS_PER_SLASHINGS_VECTOR")
  ReadVector<EpochNumber, Gwei> getSlashings();

  /* ******** Attestations ********* */

  @SSZ(order = 14, maxSizeVar = "spec.MAX_EPOCH_ATTESTATIONS")
  ReadList<Integer, PendingAttestation> getPreviousEpochAttestations();

  @SSZ(order = 15, maxSizeVar = "spec.MAX_EPOCH_ATTESTATIONS")
  ReadList<Integer, PendingAttestation> getCurrentEpochAttestations();

  /* ******** Finality ********* */

  /** Bit set for every recent justified epoch. */
  @SSZ(order = 16, vectorLengthVar = "spec.JUSTIFICATION_BITS_LENGTH")
  Bitvector getJustificationBits();

  /** Previous epoch snapshot. */
  @SSZ(order = 17) Checkpoint getPreviousJustifiedCheckpoint();

  @SSZ(order = 18) Checkpoint getCurrentJustifiedCheckpoint();

  @SSZ(order = 19) Checkpoint getFinalizedCheckpoint();
  ...
  default String toStringShort(@Nullable SpecConstants spec) {
    String ret = "BeaconState["
        ...
        + ", validators: " + getValidators().size()
        + ", just/final epoch: " + getCurrentJustifiedCheckpoint().getEpoch().toString(spec) + "/" + getFinalizedCheckpoint().getEpoch().toString(spec);
    if (spec != null) {
      ret += ", latestBlocks=[...";
